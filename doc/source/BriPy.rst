...
.. function:: track_nucs([isolist])
    ...
    calling `track_nucs()` becomes necessary at the beginning of most Bright codes so that the components 
    ...
    However, using `isoname` functions you can specify isotopic lists in a more natural way::
        ...
        bright.track_nucs(isolist)

        #Get track_nucs
        bright.track_nucs()  #returns the list [922350, 942390, 10010, 80160]
...
.. function:: load_track_nucs_hdf5(filename[, dataset='', clear=False])
...
.. function:: load_track_nucs_text(filename[, clear=False])
